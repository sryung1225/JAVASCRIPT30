...
// -----------------------------------------------------
{
  ...
  const snap = document.querySelector('.snap') as HTMLAudioElement; // 촬영 시 들릴 효과음
  const strip = document.querySelector('.strip') as HTMLDivElement; // 갤러리
  ...
  function greenScreen(pixels: ImageData) {
    const levels: { [key: string]: number } = {}; // 최대/최소 범위 저장

    document.querySelectorAll('.rgb input').forEach((input) => {
      const inputElement = input as HTMLInputElement;
      levels[inputElement.name] = Number(inputElement.value);
    });
  ...
  (window as any).toggleRedEffect = toggleRedEffect;
  (window as any).toggleRgbSplit = toggleRgbSplit;
