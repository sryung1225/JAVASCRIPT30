...
function hasAdult(people: Person[]) {
...
function isEveryoneAdult(people: Person[]) {
  const answer = people.every((person) => currentYear - person.year + 1 >= 19);
  console.log(answer);
}

// 3. ID에 해당되는 comment 찾기
function findComment(comments: Comment[], id: number) {
  const comment = comments.find((comment) => comment.id === id);
  if (comment) console.log(comment.text);
}

// 4. ID에 해당되는 comment 제거하기
function deleteComment(comments: Comment[], id: number) {
