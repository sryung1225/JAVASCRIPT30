...
// -----------------------------------------------------
const holes = document.querySelectorAll('.hole') as NodeListOf<HTMLDivElement>; // 구멍
const scoreBoard = document.querySelector('.score') as HTMLSpanElement;
const moles = document.querySelectorAll('.mole') as NodeListOf<HTMLDivElement>; // 두더지

let lastHole: HTMLDivElement; // 가장 최근 두더지가 나왔던 구멍 : 연속 등장 방어
let timeUp: boolean = false; // 게임 중 여부
let score: number = 0; // 점수
...
function peep() {
  ...
  setTimeout(() => {
    ...
    if (!timeUp) peep(); // 잔여 시간이 남아있으면 랜덤 두더지 재등장
