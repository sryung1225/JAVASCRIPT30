...
// -----------------------------------------------------
{
  ...
  function handleResult(e: SpeechRecognitionEvent) {
    ...
    const poopScript: string = transcript.replace(/바보|멍청이|똥/gi, '💩');
  ...
  recognition.addEventListener('result', handleResult as EventListener); // 음성 인식되면 동작
  recognition.addEventListener('end', recognition.start as EventListener); // 음성 인식 종료되면 해당 회차 종료 후 재시작
