...
function born1500s(inventors: Inventor[]) {
...
// 2. inventors 이름 나열하기
function callName(inventors: Inventor[]) {
  const answer = inventors.map(
...
function sortFirstBirth(inventors: Inventor[]) {
  const answer = [...inventors].sort((a, b) => a.year - b.year);
  console.log(answer);
}

// 4. 모든 inventors가 살았던 햇수 더하기
function getYears(inventors: Inventor[]) {
  const answer = inventors.reduce(
    (acc: number, cur) => acc + (cur.passed - cur.year),
    0
  );
  console.log(answer);
}
...
function sortLongLives(inventors: Inventor[]) {
  const answer = [...inventors].sort(
...
function findBoulevards() {
  ...
  const boulevards = links.map((link) => link.textContent as string); // <a /> 내 텍스트 가져오기
  const de = boulevards.filter((boulevard) => boulevard.includes("de"));
  console.log(de);
}

// 7. people 성 순으로 나열하기
function sortLastName(people: string[]) {
...
// 8. rides 종류 별 갯수 구하기
function countRides(rides: string[]) {
  const answer = rides.reduce((acc: { [key: string]: number }, cur) => {
